...
app.get('/api/v1/towns/:id', async(req, res, next) => {
  try {
    const townId = req.params.id;
    const foundTown = await Town.findById(townId);
    res.json(foundTown);
  } catch(error) {
    next(error);
  }
...
app.delete('/api/v1/towns/:id', async(req, res, next) => {
  try {
    const townId = req.params.id;
    const deletedTown = await Town.delete(townId);
    res.json(deletedTown);
  } catch(error) {
    next(error);
  }
});

app.put('/api/v1/towns/:id', async(req, res, next) => {
  try {
    const townId = req.params.id;
    const updatedTown = await Town.update(townId, req.body);
